...
Progateの学習内容をベースに、**オブジェクト指向（OOP）**と**MySQLデータベース**を組み合わせ、より実務に近い動的なシステムへと進化させました。
...
## 💡 こだわったポイント
* **データベース連携**: 従来、配列で管理していた情報をMySQLへ移行。SQLを用いて効率的にデータを取得する構造に改善しました。
* **オブジェクト指向の活用**:
  * **クラスの継承**: `Menu` クラスを親とし、`Plant` と `Goods` で継承を行うことで共通処理を効率化。
  * **カプセル化**: プロパティに `private` / `protected` を使用し、安全性を向上。
  * **動的な条件分岐**: `instanceof` を用いて、インスタンスの種類に応じたレイアウトの出し分けを実装。
...
### 2. 環境設定
database.php をご自身の環境に合わせて編集してください（Host: 127.0.0.1, Port: 3306 など）。
